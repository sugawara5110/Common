...
//ライト計算
    ...
    {
        tmp = PointLightCom(g_Specular.xyz, g_Diffuse.xyz, Ambient, N,
                            g_LightPos[i], input.wPos.xyz, g_Lightst[i], g_LightColor[i].xyz, g_C_Pos.xyz, g_DispAmount.z);
        Out.Diffuse += tmp.Diffuse;
        Out.Specular += tmp.Specular;
    }

//平行光源計算
    tmp = DirectionalLightCom(g_Specular.xyz, g_Diffuse.xyz, Ambient, N,
...
//最後にテクスチャの色を掛け合わせる
    ...
    float3 spe = Out.Specular * Tspe.xyz;
