...
bool SkinnedCom::createDescHeap(D3D12_GPU_VIRTUAL_ADDRESS ad3, UINT ad3Size) {
	...
	UINT resSize = sizeof(SkinMeshHelper::Skin_VERTEX);
	...
	UINT size = pd->dpara.Vview.get()->VertexBufferByteSize / sizeof(SkinMeshHelper::Skin_VERTEX);
...
bool SkinnedCom::createParameter(int comIndex) {
	...
	for (int i = 0; i < NumMaterial; i++) {
		...
		if (FAILED(dxI.IndexBufferGPU.createDefaultResourceBuffer(dxI.IndexBufferByteSize))) {
			Dx_Util::ErrorMessage("SkinnedCom::createParameter Error!!");
			return false;
		}
		...
		for (int j = 0; j < 2; j++) {
			...
			UINT vCnt = pd->dpara.Vview.get()->VertexBufferByteSize / sizeof(SkinMeshHelper::Skin_VERTEX);
